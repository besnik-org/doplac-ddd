...
class DomainSupport
{
    ...
    protected function data(): array
    {
        ...
        ksort($autoload);
        $seeders = [];
        $domains = ["app" => ['title' => 'app', 'namespace' => 'App\\', 'real_path' => 'app']];
        $factories = [];
        ...
        foreach ($allDomainFiles as $file) {
            ...
            $domains[$title] = ['title' => $title, 'namespace' => array_keys($config['autoload'])[0], 'real_path' => 'domains' . DIRECTORY_SEPARATOR . $title . DIRECTORY_SEPARATOR . 'app',];

//            dd($domains);
        }


        return ['factories' => $factories, 'domains' => $domains, 'seeders' => $seeders,];
    }
